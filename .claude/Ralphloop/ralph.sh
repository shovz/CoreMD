...
if ! command -v claude &>/dev/null; then
    CLAUDE_EXT=$(ls /c/Users/USER/.vscode/extensions/ 2>/dev/null | grep "anthropic.claude-code" | sort -V | tail -1)
    if [ -n "$CLAUDE_EXT" ]; then
        export PATH="/c/Users/USER/.vscode/extensions/${CLAUDE_EXT%/}/resources/native-binary:$PATH"
    fi
fi
